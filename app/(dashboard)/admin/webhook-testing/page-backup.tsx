...
export default function WebhookTestingPage() {
  ...
  // Check if user is authorized (QA/admin)
  const isAuthorized = () => {
    if (!user || user.user_type !== 'admin') {
      return false
    }
    // Check permissions or role_level for QA access
    const adminProfile = userProfile as Admin | null
    if (!adminProfile) {
      return false
    }
    // Check if permissions include 'qa' or role_level is appropriate
    const permissions = adminProfile.permissions || {}
    return permissions.qa === true || adminProfile.role_level === 1 || adminProfile.role_level === 2
  }
