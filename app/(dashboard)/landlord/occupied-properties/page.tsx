      ...
      <CardContent className="p-5">
        <div className="flex flex-col lg:flex-row gap-5">
          ...
          <div className="flex lg:flex-col items-start gap-3">
            <Checkbox 
              checked={isSelected} 
              onCheckedChange={() => onSelect(agreement.id)} 
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mb-4 p-3 bg-slate-50 rounded-lg border border-slate-100">
              <Avatar className="h-10 w-10 border border-orange-100">
                <AvatarImage src={tenant?.avatar_url ?? `${DEFAULT_AVATAR}${tenant?.full_name ?? "tenant"}`} />
              ...
              <Link href={`/landlord/messages?conversation=${tenant?.id}`}>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-100 flex-wrap">
              ...
              <Link href={`/landlord/maintenance?property=${property?.id}`}>
                <Button size="sm" variant="outline" className="border-orange-300 text-orange-700 hover:bg-orange-50 hover:border-orange-400 h-9">
...
export default function LandlordOccupiedPropertiesPage() {
  ...
  const fetchActiveAgreements = useCallback(async () => {
    try {
      ...
      console.error("[LandlordOccupied] fetch error:", error)
  ...
  const toggleSelectAll = () => {
    if (selectedAgreements.length === activeAgreements.length) {
      setSelectedAgreements([])
    } else {
      setSelectedAgreements(activeAgreements.map(a => a.id))
    }
  }
  ...
  const sendBulkRentReminders = () => {
    toast.success(`Rent reminder sent to ${selectedAgreements.length} tenant(s)!`)
    setSelectedAgreements([])
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-900 mb-2">Loading Occupied Properties</h3>
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-slate-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-slate-600">
                Manage all your currently rented properties and tenants
              </p>
            </div>
          </div>
        </div>
        ...
        {activeAgreements.length > 0 && (
          <div className="mb-6 p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <Checkbox 
                checked={selectedAgreements.length === activeAgreements.length && activeAgreements.length > 0}
                onCheckedChange={toggleSelectAll}
                className="h-5 w-5"
              />
              <span className="text-sm text-slate-600">
                {selectedAgreements.length === 0 
                  ? "Select properties for bulk actions" 
                  : `${selectedAgreements.length} property(ies) selected`}
              </span>
            </div>
        ...
        <div className="grid gap-4 grid-cols-1 md:grid-cols-3 mb-8">
          <Card className="border-orange-200 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-lg transition-all duration-300">
            <CardContent className="p-5 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-0.5">Occupied Properties</p>
        ...
        {activeAgreements.length > 0 && (
          <div className="mb-6">
            <div className="relative max-w-md">
              ...
              <input
                type="text"
                placeholder="Search by property name, location, or tenant..."
                value={searchTerm}
          ...
          <Card className="border-orange-200 bg-white/80 backdrop-blur-sm shadow-sm hover:shadow-lg transition-all duration-300">
            <CardContent className="p-14 text-center">
              ...
              <p className="text-slate-600 mb-8 max-w-sm mx-auto">
                {activeAgreements.length === 0
                  ? "You don't have any currently rented properties yet. Once a tenant signs an agreement, it will appear here."
          ...
          <div className="space-y-4">
            {filteredAgreements.map((agreement) => (
              <OccupiedPropertyCard 
                key={agreement.id} 
                agreement={agreement}
                isSelected={selectedAgreements.includes(agreement.id)}
                onSelect={toggleSelectAgreement}
              />
            ))}
          </div>
        )}
