...
export async function proxy(request: NextRequest) {
  ...
  // ─── BUG-001 FIX: Explicit early-exit for /auth/* routes ────────────────────────
  // Cookies from the callback flow commit AFTER the redirect response is sent.
  // If middleware runs on /onboarding before cookies are readable, it redirects
  // to /signin (no session) causing the flash-through-root Symptom.
  if (pathname.startsWith('/auth/')) {
    console.log('✅ Auth route - skipping middleware')
    return NextResponse.next()
  }

  // ── FIX: Honour the just-registered-type cookie set by the Google callback ──
  // When a previously-deleted user re-registers via Google, their auth JWT still
  // carries the OLD user_type from the deleted account. The DB fetch that corrects
  // this runs client-side AFTER the first page render — meaning this middleware
  // would read the wrong type and redirect them (e.g. landlord → /properties).
  // The Google callback stamps a 2-minute cookie with the freshly-resolved type.
  // We trust it here so the first /onboarding/* request is let through.
  const justRegisteredType = request.cookies.get('nulo_just_registered_type')?.value
  if (justRegisteredType) {
    console.log('🍪 [MIDDLEWARE] nulo_just_registered_type cookie present:', justRegisteredType)
    if (
      (justRegisteredType === 'landlord' && pathname.startsWith('/onboarding/landlord')) ||
      (justRegisteredType === 'tenant'   && pathname.startsWith('/onboarding/tenant'))
    ) {
      console.log('✅ [MIDDLEWARE] Allowing just-registered user through to onboarding:', pathname)
      return NextResponse.next()
    }
  }
  ...
  if (profile.user_type === 'landlord') {
    ...
    // During email verification, the session may briefly have undefined onboarding_completed.
    // Check if email is just now verified and user has landload user_type - allow through to onboarding.
    // This prevents the "flicker to homepage" issue where users get redirected incorrectly.
    if (profile?.onboarding_completed === undefined) {
      // ✅ FIX: If email is verified and user_type is landlord, allow to onboarding
      // This prevents redirect to /properties (tenant fallback) during verification flow
      console.log('⚠️ [MIDDLEWARE] onboarding_completed is undefined - checking if this is post-verification flow')

      // If we have a valid landlord user_type and email is verified, allow onboarding access
      if (emailVerified) {
        console.log('✅ [MIDDLEWARE] Email verified + landlord user_type - allowing onboarding access')
        return NextResponse.next()
      }
